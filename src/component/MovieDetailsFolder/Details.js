...
const Details = () => {
        ...
        if(movieDetails!==[]){
            ...
            .then((data) => {
                if(!data.errors){
                    ...
                    (obj !== undefined) && setTrailerLink(obj.key);
                }
                else{setTrailerLink("");
                }
            })
    ...
    const storedWatchListMovie = watchList.find(m => m.id === movieDetails.id);
    const storedWatchedMovie = watched.find( m => m.id === movieDetails.id);
    const disabledWatchlistButton = !!(storedWatchListMovie || storedWatchedMovie)
    const disabledWatchedButton = !!(storedWatchedMovie)
    return (
        <div> {(movieDetails&& movieDetails.genre_ids) ? 
        <div style={{backgroundImage:`url("https://image.tmdb.org/t/p/w500${movieDetails.backdrop_path}")`,width:'100%',backgroundRepeat:'no-repeat',backgroundSize:'100%',height:'100%'}}>
             <div className="detailsContent row">
                ...
                <div className="col-xs-8 col-sm-6 col-md-7 col-lg-8">
                    <h2 className="detailsTitle">{movieDetails.title}</h2>
                    <span> Release Date: {movieDetails.release_date.toLocaleString()}</span> 
                    <span className="genreWrapper"> 
                    {
                    ...
                    <span className="details_actions">
                        ...
                        </span>
                        {
                            (trailerLink!=="") && 
                            <span >
                                <button className="addButton"><a href={`https://www.youtube.com/watch?v=${trailerLink}`} target="_blank" className="trailerLink"><img src="https://img.icons8.com/color/24/ffffff/youtube-play.png"/> Trailer</a></button>
                            </span>
                        }
                    </span>
                    <div>
